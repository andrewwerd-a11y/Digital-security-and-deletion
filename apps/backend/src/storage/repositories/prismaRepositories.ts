...
export class PrismaQueueRepository implements QueueRepository {
  ...
  async getById(id: string): Promise<QueueRecord | null> {
  ...
  async update(id: string, patch: Partial<Pick<QueueRecord, "state" | "blockReason" | "detail">>): Promise<QueueRecord> {
    const row = await prisma.queueItem.update({ where: { id }, data: patch });
    return {
      id: row.id,
      queueType: row.queueType,
      taskId: row.taskId,
...
export class PrismaPolicyDecisionRepository implements PolicyDecisionRepository {
  ...
  async getById(id: string): Promise<PolicyDecisionRecord | null> {
    const row = await prisma.policyDecision.findUnique({ where: { id } });
    if (!row) return null;
    return {
      id: row.id,
      workflowId: row.workflowId,
...
export class PrismaActionRepository implements ActionRepository {
  async create(record: Omit<ActionRecord, "id" | "createdAt">): Promise<ActionRecord> {
    const row = await prisma.action.create({
      data: {
        ...
        actionCandidateSummary: record.actionCandidateSummary ?? null,
    ...
    return {
      ...
      actionClass: row.actionClass,
      actionType: row.actionType,
      actionCandidateSummary: row.actionCandidateSummary,
  ...
  async update(id: string, patch: Partial<Pick<ActionRecord, "state" | "resultSummary" | "errorMessage" | "completedAt">>): Promise<ActionRecord> {
    const row = await prisma.action.update({ where: { id }, data: patch });
    return {
      id: row.id,
      actionClass: row.actionClass,
      actionType: row.actionType,
...
export class PrismaVerificationRepository implements VerificationRepository {
  ...
  async listByAction(actionId: string): Promise<VerificationRecord[]> {
    const rows = await prisma.verification.findMany({ where: { actionId }, orderBy: { createdAt: "desc" } });
    return rows.map((row) => ({
      id: row.id,
      actionId: row.actionId,
      state: row.verificationState,
  ...
  async listAll(): Promise<VerificationRecord[]> {
    const rows = await prisma.verification.findMany({ orderBy: { createdAt: "desc" } });
    return rows.map((row) => ({
      id: row.id,
      actionId: row.actionId,
      state: row.verificationState,
...
export class PrismaAuditRepository implements AuditRepository {
  async create(record: AuditRecord): Promise<AuditRecord> {
    const row = await prisma.auditEvent.create({ data: { eventType: record.eventType } });
    return { id: row.id, eventType: row.eventType };
  }

  async listAll(): Promise<AuditRecord[]> {
    const rows = await prisma.auditEvent.findMany({ orderBy: { createdAt: "desc" } });
    return rows.map((row) => ({ id: row.id, eventType: row.eventType }));
  }
}
export class PrismaReportRepository implements ReportRepository {
  async create(record: Omit<ReportRecord, "id" | "createdAt">): Promise<ReportRecord> {
    ...
    return { id: row.id, reportType: row.reportType, title: row.title, summary: row.summary, payloadRef: row.payloadRef, createdAt: row.createdAt };
  }

  async listAll(): Promise<ReportRecord[]> {
    const rows = await prisma.report.findMany({ orderBy: { createdAt: "desc" } });
    return rows.map((row) => ({ id: row.id, reportType: row.reportType, title: row.title, summary: row.summary, payloadRef: row.payloadRef, createdAt: row.createdAt }));
  }

  async getById(id: string): Promise<ReportRecord | null> {
    const row = await prisma.report.findUnique({ where: { id } });
    return row ? { id: row.id, reportType: row.reportType, title: row.title, summary: row.summary, payloadRef: row.payloadRef, createdAt: row.createdAt } : null;
  }

  async listByType(reportType: string): Promise<ReportRecord[]> {
    const rows = await prisma.report.findMany({ where: { reportType }, orderBy: { createdAt: "desc" } });
    return rows.map((row) => ({ id: row.id, reportType: row.reportType, title: row.title, summary: row.summary, payloadRef: row.payloadRef, createdAt: row.createdAt }));
  }
}

export class PrismaResidualRiskRepository implements ResidualRiskRepository {
  async create(record: Omit<ResidualRiskRecord, "id" | "createdAt">): Promise<ResidualRiskRecord> {
    const row = await prisma.residualRiskRecord.create({ data: record });
    return {
      id: row.id,
      scopeKey: row.scopeKey,
      severity: row.severity,
      status: row.status as ResidualRiskRecord["status"],
      summary: row.summary,
      detail: row.detail,
      createdAt: row.createdAt
    };
  ...
  async listByScope(scopeKey: string): Promise<ResidualRiskRecord[]> {
    const rows = await prisma.residualRiskRecord.findMany({ where: { scopeKey }, orderBy: { createdAt: "desc" } });
    return rows.map((row) => ({
      id: row.id,
      scopeKey: row.scopeKey,
      severity: row.severity,
  ...
  async listAll(): Promise<ResidualRiskRecord[]> {
    const rows = await prisma.residualRiskRecord.findMany({ orderBy: { createdAt: "desc" } });
    return rows.map((row) => ({
      id: row.id,
      scopeKey: row.scopeKey,
      severity: row.severity,
