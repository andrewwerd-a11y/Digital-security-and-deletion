...
export class DiscoveryOrchestrator {
  constructor(
    ...
    private readonly discoveryRunRepository: DiscoveryRunRepository,
    private readonly discoveryFindingRepository: DiscoveryFindingRepository,
  ...
  async startLocalDiscovery(input: StartDiscoveryInput) {
    ...
    const run = await this.discoveryRunRepository.create({
      workflowId: workflow.id,
    ...
    try {
      ...
      for (const finding of findings) {
        const createdFinding = await this.discoveryFindingRepository.create({
      ...
      await this.discoveryRunRepository.update(run.id, {
  ...
  async getDiscoverySummary(discoveryRunId: string) {
    const run = await this.discoveryRunRepository.getById(discoveryRunId);
    const findings = await this.discoveryFindingRepository.listByRun(discoveryRunId);
