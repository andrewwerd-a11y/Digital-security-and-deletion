...
export async function buildServer() {
  ...
  const verificationService = new VerificationService(repositories.verificationRepository);
  const reportService = new ReportService(
    ...
    repositories.residualRiskRepository
  );
  ...
  app.post("/api/verification/record", async (request, reply) => {
    const payload = request.body as {
      actionId: string;
    ...
    const record = await verificationService.recordOutcome(payload);
    return reply.code(201).send(record);
  });

  app.get("/api/verification/summary", async () => verificationService.getSummary());

  app.post("/api/reports/generate", async (request, reply) => {
    const payload = request.body as { reportType: string; title: string; summary: string; payload: unknown };
    const report = await reportService.createReport(payload);
    return reply.code(201).send(report);
  });

  app.post("/api/reports/workspace/generate", async () => reportService.generateWorkspaceReports());
  app.get("/api/reports", async () => reportService.listReports());
  app.get("/api/reports/:id", async (request) => {
    const params = request.params as { id: string };
    return reportService.getReport(params.id);
  });

  app.post("/api/residual-risks", async (request, reply) => {
    const payload = request.body as { scopeKey: string; summary: string; detail?: string; severity?: "LOW" | "MEDIUM" | "HIGH" | "CRITICAL" };
    const created = await reportService.createResidualRisk(payload as any);
    return reply.code(201).send(created);
  });

  app.get("/api/residual-risks", async (request) => {
    const query = request.query as { scopeKey?: string };
    return reportService.listResidualRisks(query.scopeKey);
  });

  app.get("/api/audit-timeline", async () => reportService.getAuditTimeline());

  app.post("/api/rebuild/plan", async (request, reply) => {
    const payload = request.body as { manifestId: string; excludedItemIds?: string[] };
    const plan = await rebuildService.generatePlan(payload.manifestId, payload.excludedItemIds ?? []);
    return reply.code(201).send(plan);
  });

  app.get("/api/rebuild/plan/:manifestId", async (request) => {
    const params = request.params as { manifestId: string };
    const model = await repositories.readModelRepository.getByTypeAndKey("rebuild_plan", params.manifestId);
    return model ? JSON.parse(model.payload) : null;
  });

  app.post("/api/rebuild/start", async (request, reply) => {
    const payload = request.body as { manifestId: string; excludedItemIds?: string[] };
    const started = await rebuildService.startRebuild(payload.manifestId, payload.excludedItemIds ?? []);
    return reply.code(202).send(started);
  });

  app.post("/api/rebuild/validate", async (request, reply) => {
    const payload = request.body as { manifestId: string; validatedItemIds: string[]; unverifiableItemIds: string[] };
    const summary = await rebuildService.validatePostRestore(payload.manifestId, payload.validatedItemIds, payload.unverifiableItemIds);
    return reply.code(201).send(summary);
  });

  app.get("/api/rebuild/validation/:manifestId", async (request) => {
    const params = request.params as { manifestId: string };
    const model = await repositories.readModelRepository.getByTypeAndKey("post_restore_validation", params.manifestId);
    return model ? JSON.parse(model.payload) : null;
  });
